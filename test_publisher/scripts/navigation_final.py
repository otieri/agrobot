...
from geometry_msgs.msg import Twist, PoseWithCovarianceStamped, Pose, Point, Quaternion
...
class NaviIntegration:
    ...
    def __init__(self):
        ...
        self.THETA_WEIGHT = 0.5  # 0.05: Slightly rotate while driving # 0.5: Farily rotate
        self.NEAR_AREA_DIST = 0.4
        ...
        for yawangle in self.yaweulerangles_seq:
            # Unpacking the quaternion list and passing it as arguments to Quaternion message constructor
            self.quat_seq.append(Quaternion(*(quaternion_from_euler(0, 0, yawangle * math.pi / 180, axes='sxyz'))))

        # Returns a list of lists [[point1], [point2],...[pointn]]
        n = 3
        points = [self.points_seq[i:i + n] for i in range(0, len(self.points_seq), n)]
        for point in points:
    ...
    def lidarCallback(self, lidarscan):
        ...
        if self.dot_dir > 0:  # Forward
            points = [list(cloud_point) for cloud_point in cloud_points if
                      cloud_point[0] <= 10.0 and cloud_point[2] < self.MAX_HEIGHT and cloud_point[2] > self.MIN_HEIGHT]
        else:  # Backward
            points = [list(cloud_point) for cloud_point in cloud_points if
                      cloud_point[0] >= -10.0 and cloud_point[2] < self.MAX_HEIGHT and cloud_point[2] > self.MIN_HEIGHT]
                ...
                if self.dot_dir < 0:  # When the robot needs a rotation in place to reach the goal position
                    ...
                    if self.dist_amcl_goal > self.NEAR_AREA_DIST:  # When the robot is near the goal point: "Fine" controller
                        ...
                        while ((rospy.Time.now() < endTime) & self.rotation_trigger1 == True):
                            ...
                            print("Rotating in place (forward), angular velocity: ", vel_msg.angular.z,
                                  rospy.Time.now())
